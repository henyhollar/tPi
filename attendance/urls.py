...
urlpatterns = [
    ...
    url(r'^attendance_list/(?P<course_code>[A-Z0-9]{6})/(?P<year>\d{4})/(?P<month>[A-Za-z]{3})/(?P<day>[0-9]{2})/$',
        views.Get_Who_Attended.as_view(), name='get_who_attended_with_date'),
    url(r'^stop_class/$', views.StopActiveClass.as_view(), name='stop_class'),
    url(r'^clear_attendance/(?P<course_code>[A-Z0-9]{6})/$', views.clear_attendance),
